...
exports.markPromptSent = markPromptSent;
...
function markPromptSent() {
    ...
    (0, logger_1.log)(`[stateReader] Prompt sent, will check for errors after 30s`);
}
async function readAntigravityState() {
    ...
    if (lastAgentWorking === null || agentWorking !== lastAgentWorking) {
        ...
        (0, logger_1.log)(agentWorking ? `⏳ Agent WORKING (step ${currentStepIndex})` : `✅ Agent IDLE (step ${currentStepIndex})${errStr}`);
            ...
            if (currentStepIndex !== lastStepIndex) {
                ...
                if (!agentWorking && promptSentAt && (Date.now() - promptSentAt >= ERROR_CHECK_DELAY_MS)) {
                    ...
                    if (errors > 0) {
                        ...
                        (0, logger_1.log)(`[stateReader] 💀 Detected ${errors} fatal error(s) since last step`);
                    }
